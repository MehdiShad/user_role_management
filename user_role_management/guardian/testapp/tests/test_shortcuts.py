...
from user_role_management.guardian.compat import get_user_permission_full_codename
from user_role_management.guardian.shortcuts import assign
...
from user_role_management.guardian.shortcuts import get_user_perms
from user_role_management.guardian.shortcuts import get_group_perms
...
from user_role_management.guardian.exceptions import MixedContentTypeError
from user_role_management.guardian.exceptions import NotUserNorGroup
from user_role_management.guardian.exceptions import WrongAppError
...
from user_role_management.guardian.models import Permission, CompanyGroups
...
class MultipleIdentitiesOperationsTest(ObjectPermissionTestCase):
    ...
    def setUp(self):
        ...
        self.groups_list = jim_group, bob_group = [
            CompanyGroups.objects.create(name='jimgroup'),
            CompanyGroups.objects.create(name='bobgroup')
        ]
        jim_group.user_set.add(jim)
        bob_group.user_set.add(bob)
        self.users_qs = User.objects.exclude(username='AnonymousUser')
        self.groups_qs = CompanyGroups.objects.all()
...
class GetUsersWithPermsTest(TestCase):
    ...
    def setUp(self):
        ...
        self.group1 = CompanyGroups.objects.create(name='group1')
        self.group2 = CompanyGroups.objects.create(name='group2')
        self.group3 = CompanyGroups.objects.create(name='group3')
...
class GetGroupsWithPerms(TestCase):
    ...
    def test_filter_by_contenttype(self):
        ...
        group = CompanyGroups.objects.create(pk=1042, name='group')
...
class GetObjectsForUser(TestCase):
    ...
    def setUp(self):
        ...
        self.group = CompanyGroups.objects.create(name='group')
    ...
    def test_empty_perms_sequence(self):
        objects = get_objects_for_user(self.user, [], CompanyGroups.objects.all())
        self.assertEqual(
            set(objects),
            set()
        )
    ...
    def test_simple(self):
        ...
        groups = [CompanyGroups.objects.create(name=name) for name in group_names]
        for group in groups:
    ...
    def test_groups_perms(self):
        group1 = CompanyGroups.objects.create(name='group1')
        group2 = CompanyGroups.objects.create(name='group2')
        group3 = CompanyGroups.objects.create(name='group3')
    ...
    def test_has_any_permissions(self):
        ...
        objects = get_objects_for_user(self.user, [], CompanyGroups)
        self.assertEqual(len(objects), len(groups))
        self.assertTrue(isinstance(objects, QuerySet))
        self.assertEqual(
            set(objects),
    ...
    def test_has_any_group_permissions(self):
        ...
        objects = get_objects_for_group(self.group, [], CompanyGroups)
        self.assertEqual(len(objects), len(groups))
        self.assertTrue(isinstance(objects, QuerySet))
        self.assertEqual(
            set(objects),
