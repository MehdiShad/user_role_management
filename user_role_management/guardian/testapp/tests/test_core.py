...
from user_role_management.guardian.models import UserObjectPermission, GroupObjectPermission
from user_role_management.guardian.shortcuts import assign_perm
from user_role_management.guardian.management import create_anonymous_user
from user_role_management.guardian.utils import evict_obj_perms_cache

from user_role_management.guardian.testapp.models import Project, ProjectUserObjectPermission, ProjectGroupObjectPermission
...
class ObjectPermissionCheckerTest(ObjectPermissionTestCase):
    ...
    def test_cache_for_queries_count(self):
        ...
        try:
            ...
            if 'user_role_management.guardian.testapp' in settings.INSTALLED_APPS:
                expected = 5
            else:
            ...
            # Checking for other permission but for CompanyGroups object again
            ...
            new_group = CompanyGroups.objects.create(name='new-group')
            query_count = len(connection.queries)
    ...
    def test_get_perms(self):
        group = CompanyGroups.objects.create(name='group')
    ...
    def test_prefetch_group_perms_direct_rel(self):
        ...
        try:
            ...
            ContentType.objects.clear_cache()
            group = CompanyGroups.objects.create(name='new-group')
            projects = \
                [Project.objects.create(name='Project%s' % i)
                    for i in range(3)]
