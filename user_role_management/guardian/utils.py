...
def get_identity(identity):
    ...
    :param identity: either ``User`` or ``CompanyGroups`` instance
    ...
    **Examples**::
       ...
       >>> group = CompanyGroups.objects.create(name='users')
       >>> get_identity(group)
       (None, <CompanyGroups: users>)
       ...
       ...
       NotUserNorGroup: User/AnonymousUser or CompanyGroups instance is required (got )
    ...
    if isinstance(identity, QuerySet):
        ...
        elif identity_model_type == CompanyGroups:
            return None, identity

    # get identity from first element in list
    if isinstance(identity, list) and isinstance(identity[0], get_user_model()):
        return identity, None
    if isinstance(identity, list) and isinstance(identity[0], CompanyGroups):
        return None, identity

    if isinstance(identity, get_user_model()):
        return identity, None
    if isinstance(identity, CompanyGroups):
        return None, identity

    raise NotUserNorGroup("User/AnonymousUser or CompanyGroups instance is required "
                          "(got %s)" % identity)
...
def get_obj_perm_model_by_conf(setting_name):
    ...
    try:
        ...
        # setting_value = 'guardian.GroupObjectPermission'
        df = django_apps.get_model(setting_value, require_ready=False)
...
def get_group_obj_perms_model(obj = None):
    """
    Returns model class that connects given ``obj`` and CompanyGroups class.
    ...
    """
    from user_role_management.guardian.models import GroupObjectPermissionBase
