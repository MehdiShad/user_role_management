...
class GetIdentityTest(ObjectPermissionTestCase):
    ...
    def test_group(self):
        ...
        self.assertTrue(isinstance(group, CompanyGroups))
    ...
    def test_multiple_group_qs(self):
        user, group = get_identity(CompanyGroups.objects.all())
...
class GetGroupObjPermsModelTest(TestCase):
    ...
    def test_group_model(self):
        # this test assumes that there were no direct obj perms model to CompanyGroups
        # model defined (i.e. while testing guardian app in some custom
        # project)
        self.assertEqual(get_group_obj_perms_model(CompanyGroups),
                         GroupObjectPermission)
