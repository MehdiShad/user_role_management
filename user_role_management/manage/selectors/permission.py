...
def get_user_permissions(request: HttpRequest) -> Dict[str, Literal['is_success', True, False]]:
    try:
        ...
        access_processes = GroupObjectPermission.objects.filter(content_type_id__in=[process_model_id.id], group_id__in=company_groups)
        access_actions = GroupObjectPermission.objects.filter(content_type__in=[action_model_id.id], group_id__in=company_groups)
