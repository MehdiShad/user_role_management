...
def check_settings(app_configs, **kwargs):
    ...
    """
    checks = []
    if 'user_role_management.guardian.backends.ObjectPermissionBackend' not in settings.AUTHENTICATION_BACKENDS:
        msg = ("Guardian authentication backend is not hooked. You can add this in settings as eg: "
               "`AUTHENTICATION_BACKENDS = ('django.contrib.auth.backends.ModelBackend', "
               "'guardian.backends.guardian.backends.ObjectPermissionBackend')`.")
